    ...
    <div class="page-and-modal">
        <main>
            ...
            <div class="results-container">
                <div class="card-link" v-for="result in displayedResults" :key="result.item.value" @click="gogo(result.uniqueId.value)">
...
const filter = ref(route.query.title || '');
...
watchEffect(() => {
    const query = filter.value ? { ...route.query, title: filter.value } : { ...route.query };
    console.log(query)
    router.replace({ query });
});

function gogo(id) {
    const query = { ...route.query, record: id } ;
    router.replace({ query });
}
